...
// Download dari url jadi Readable stream (dipakai buat kirim media gede
// kayak video, biar ga di-buffer penuh di memory)
...
export const sendAsSticker = async (axmisu, jid, buffer, quoteEvent, options = {}) => {
  const raw = await mediaToWebp(buffer, options.mime || '');
  const final = await addExif(raw, options.packname, options.author);
  return axmisu.message.send(jid, { type: 'sticker', media: final, mimetype: 'image/webp' }, { quote: quoteEvent });
};